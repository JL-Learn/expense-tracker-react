...
const Balance = () => {
  ...
  const labels = [...new Set([...transactions].sort((a, b) => a.date?.toDate() - b.date?.toDate()).map(transaction => getMonth(transaction.date)))];

  const incomes = getBalance(labels, 'income', transactions);
  const expenses = getBalance(labels, 'expense', transactions);

  // console.log('Soma das entradas:', incomes);
  // console.log('Soma das despesas:', expenses);
